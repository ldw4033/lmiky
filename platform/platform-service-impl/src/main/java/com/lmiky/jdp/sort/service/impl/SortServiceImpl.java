...
public class SortServiceImpl extends BaseServiceImpl implements SortService {
    ...
    public <T extends BaseSortPojo> void sort(Class<T> sortPojoClass, String[] sortedIds) {
        try {
    ...
    public <T extends BaseSortPojo> void sort(Class<T> sortPojoClass, String[] sortedIds,
        ...
        try {
            ...
            for (String id : sortedIds) {
                this.update(sortPojoClass, BasePojo.POJO_FIELD_NAME_ID, Long.valueOf(id),
                        BaseSortPojo.POJO_FIELD_NAME_SORT, sortValue);
                sortValue--;
            }
        } catch (DatabaseException e) {
            throw new ServiceException(e.getMessage());
