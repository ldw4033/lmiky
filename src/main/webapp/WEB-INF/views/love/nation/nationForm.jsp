...
<body>
	...
	<!-- form id="mainForm" action="<c:url value="/user/save.shtml"/>" method="post" -->
		<form:form id="mainForm" action="/love/nation/save.shtml" method="post" commandName="pojo">
		...
		<table class="table-form full" >
	       	<tr>
	       		<td colspan="6" class="bg01 title">
	       			...
	       			<c:choose>
	       				<c:when test="${param[httpParamOpenMode] == createOpenMode }">添加民族</c:when>
	       				<c:when test="${param[httpParamOpenMode] == editOpenMode }">修改民族</c:when>
	       				<c:otherwise>民族信息</c:otherwise>
	       			</c:choose>
	       		</td>
	       	</tr>
